...
import { migrateLegacyAgentJobNames } from '../../../src/jobs/migrateLegacyAgentJobNames.js'
...
/* Regression coverage for the rename this migration exists to do safely: a job document
   ...
   invisible to every instance forever once nothing define()s that name anymore (see
   migrateLegacyAgentJobNames.ts for the full mechanism). setupIntTest() does not clear
   agendaJobs, so each test manages its own cleanup. */
setupIntTest()

const agendaJobs = () => mongoose.connection.db!.collection('agendaJobs')

describe('migrateLegacyAgentJobNames', () => {
  beforeEach(async () => {
    await agenda.cancel({})
  })
  afterAll(async () => {
  ...
  test('renames legacy per-agent job names to the generic name, preserving data.agentId', async () => {
    ...
    const migrated = await migrateLegacyAgentJobNames()
  ...
  test('leaves already-generic-named documents untouched', async () => {
  ...
  test('is idempotent: a second run after a successful migration is a no-op', async () => {
    await agendaJobs().insertOne({
    ...
    expect(await migrateLegacyAgentJobNames()).toBe(1)
    expect(await migrateLegacyAgentJobNames()).toBe(0)
  })

  test('does not touch an unrelated job name that happens to contain the same words', async () => {
    await agendaJobs().insertOne({
    ...
    await migrateLegacyAgentJobNames()
