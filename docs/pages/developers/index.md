...
We integrate with [Promptfoo](https://www.promptfoo.dev/docs/red-team/) for red teaming our agents. To run a red team against the Event Assistant agent, execute `yarn redteam:event-assistant.` You will be asked to provide an email address. You do not need to create an account with Promptfoo. Any valid email address will work to run the test and report the results locally. **NOTE: the default redteam configuration runs several plugins with an interative jailbreak strategy. We have observed test runs taking almost two hours to complete and costing approximately $20 for LLM usage with a GPT-4o-mini model.**

This target uses a [custom Promptfoo provider](https://www.promptfoo.dev/docs/providers/custom-api/), located at `tests/utils/promptfooProvider.ts`, to create a simulated event and pass questions generated by Promptfoo to the Event Assistant for response. This provider uses the settings you have specified in your `.env` file to connect to Mongo, specify the LLM provider and model, etc. It essentially simulates running the agent as it runs in LLM Engine.

The promptfoo configuration file, located at `promptfoo/promptfoconfig-event-assistant.yaml` provides information about the type of event we are simulating (a talk about why part-time work is better) under `redteam:purpose`. This information allows Promptfoo to tailor its iterative prompting to the test scenario, asking relevant questions about the simulated event in its jailbreak attempts.

You can duplicate and/or modify the provider code and configuration file to allow for red teaming additional agents.
