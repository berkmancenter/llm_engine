...
async function parseAgentStructuredResponse(text: string, responseFormatSchema) {
  ...
  // If we have an arrayKey, the parser expects just the array, not the wrapper object
  // So we need to unwrap the JSON before parsing, then re-wrap after
  let textToParse = cleanedText
  if (arrayKey) {
    try {
      const parsedJson = JSON.parse(cleanedText)
      if (parsedJson && typeof parsedJson === 'object' && arrayKey in parsedJson) {
        // Unwrap: extract just the array
        textToParse = JSON.stringify(parsedJson[arrayKey])
        logger.debug(`Unwrapped JSON for parsing: extracted '${arrayKey}' property`)
      }
    } catch (e) {
      // If JSON.parse fails, continue with original text
      logger.debug(`Failed to pre-parse JSON for unwrapping: ${e}`)
    }
  }

  const parsed = await parser.parse(textToParse)

  // Re-wrap array if needed
...
async function getAgentStructuredResponse(llm, tools, systemPrompt, userMessage, responseSchema?) {
  ...
  if (responseSchema) {
    // Use the same unwrapping logic as parseStructuredResponse for consistency
    const { shape } = responseSchema
    const keys = Object.keys(shape)

    let schemaForInstructions
    if (keys.length === 1 && shape[keys[0]] instanceof z.ZodArray) {
      // Unwrap single-array schemas
      schemaForInstructions = shape[keys[0]]
    } else {
      schemaForInstructions = responseSchema
    }

    const parser = StructuredOutputParser.fromZodSchema(schemaForInstructions)
