...
const defaultLLMTemplates = {
  system: `You are a facilitator in a text-based chat. You consider one message at a time and your only responsibilities are to:
...
const llmTemplateVars = {
  system: [],
  user: [{ name: 'message', description: 'The message to evaluate' }]
}
...
export default verify({
  ...
  async evaluate(userMessage) {
    ...
    const llmResponse = await getChatPromptResponse(llm, this.llmTemplates.system, this.llmTemplates.user, {
      message,
      topic
    })
