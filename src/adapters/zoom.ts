  ...
  async sendMessage(message, channelConfig?) {
    ...
    if (message.bodyType === 'json') {
      ...
      if (body?.type === 'poll') {
        ...
        if (!config.nextspaceUrl) {
          logger.warn(`Poll message not sent to Zoom: NEXTSPACE_URL is not configured`)
          return
        }
        const conversationId = this.conversation._id.toString()
        const channels = this.conversation.channels ?? []
        const chatChannel = channels.find((c) => c.name === 'chat')
        const transcriptChannel = channels.find((c) => c.name?.startsWith('transcript'))
        const params = new URLSearchParams({ conversationId })
        if (transcriptChannel?.passcode) params.append('channel', `transcript,${transcriptChannel.passcode}`)
        if (chatChannel?.passcode) params.append('channel', `chat,${chatChannel.passcode}`)
        // TODO for now we assume that poll messages are only generated by the eventAssistant and thus the assistant URL is the correct one
        // may need to be updated in the future to use a configurable endpoint
        const pollUrl = `${config.nextspaceUrl}/assistant?${params.toString()}`
